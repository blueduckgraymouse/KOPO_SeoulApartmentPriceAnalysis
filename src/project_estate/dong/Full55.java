...
public class Full55  {
	...
	public static void main(String[] args) throws IOException {
		File f = new File("C:\\KOPO\\git_tracking\\project\\estate\\test.csv");
...
//						}
						...
						while (true) {
							try {
								...
								while (true) {	// 한달(한줄) 반복
									System.out.println("indexRealTrade" + indexRealTrade);
...
//									try {	// 한 달에 거래가 하나인 경우
//										System.out.println("실거래 하나인 경우");
//										realTradeDate = driver.findElement(By.xpath("//*[@id=\"tabpanel1\"]/div[4]/table/tbody/tr[" + indexRealTrade + "]/th")).getText().replaceAll("\\.", "");
//										String realTradePrice = driver.findElement(By.xpath("//*[@id=\"tabpanel1\"]/div[4]/table/tbody/tr[" + indexRealTrade + "]/td/div/div/span")).getText();
//										System.out.println(realTradeDate + "//" + realTradePrice);
//										int price = convertPrice(realTradePrice);
//										minPrice = price;
//										maxPrice = price;
//										if (indexRealTrade == 2) {		// 가장 최근 실거래 정보 저장
//											recentTradeDate = realTradeDate;
//											recentTradePrice = minPrice;
//										}
//									} catch (Exception e) {	
//										try {		// 한달에 거래가 둘 이상인 경우
//											System.out.println("실거래 둘인 경우");
//											int indexPrice = 1;
...
//													if (indexRealTrade == 2 && indexPrice == 1) {
//														recentTradeDate = realTradeDate;
...
//												indexPrice++;
//											}
//										} catch (Exception ex) {	// 거래가 없는 경우 반복문 종료
//											System.out.println("실거래 없는 경우");
//											break;
//										}
//									}
								...
								bw.write(guName + "," + dongName + "," + complexName.replaceAll(",", "") + "," + complexAddress + "," + tradeSize + "," + tradePrice + "," + recentTradeDate + "," + recentTradePrice + "," + maxTradeDate + ","  + maxTradePrice);
								System.out.println(guName + "," + dongName + "," + complexName.replaceAll(",", "") + "," + complexAddress + "," + tradeSize + "," + tradePrice + "," + recentTradeDate + "," + recentTradePrice + "," + maxTradeDate + ","  + maxTradePrice);
								System.out.println("---------------------------------------");
								bw.newLine();
								indexSize++;
							} catch (Exception e) {
	...
	private static int getRealPrice(WebDriverWait wait) {
		String data = "";
		try {
			data = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//*[@id=\"tabpanel1\"]/div[4]/table/tbody/tr[1]/td/div/div/span"))).getText();
			wait500ms();
		} catch (Exception e) {
			data = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//*[@id=\"tabpanel1\"]/div[4]/table/tbody/tr[1]/td/div/div/span[1]"))).getText();
			wait500ms();
		}
		String price = data.split("\\(")[0].replace(",", "").replace(" ", "");
		String[] datas = price.split("억");
		int result = 0;
		if (datas.length == 1) {
			result = Integer.parseInt(datas[0]) * 10000;
		} else {
			result =  Integer.parseInt(datas[0]) * 10000 + Integer.parseInt(datas[1]);
		}
		return result;
	}
