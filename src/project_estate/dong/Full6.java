...
public class Full6  {
	...
	public static void main(String[] args) throws IOException {
		File f = new File("C:\\KOPO\\git_tracking\\project\\estate\\seoul.csv");
...
//						}
					...
					/* 매매 조사 */
						...
						try {
							...
							try {	// 실거래가가 있는 경우
								...
								tempRecentRealtradePrice = getRealPrice(wait);
						...
						try {		// 범위 내에 매물이 있는 경우
							...
							wait500ms();
							String price = mintradePriceKor.split("\\(")[0].replace(",", "").replace(" ", "");
							...
							try {	// 실거래가가 있는 경우
								...
								recentRealtradePrice = getRealPrice(wait);
					...
					/* 전세 조사 */
						...
						try {	// 목표한 면적의 매물이 존재하는 경우
							...
							try {		// 범위 내에 매물이 있는 경우
								...
								wait500ms();
								String price = minRentPriceKor.split("\\(")[0].replace(",", "").replace(" ", "");
								System.out.println(price);
								String[] datas = price.split("억");
								int result = 0;
								if (datas.length == 1) {
								...
								try {	// 실거래가가 있는 경우
									...
									recentRealRentPrice = getRealPrice(wait);
	...
	private static int getSalePrice(WebDriverWait wait) {
		String data = "";
		try {
